...
public class SmartCarDriverTopology {
	...
	public static void main(String[] args) throws AlreadyAliveException, InvalidTopologyException, InterruptedException, IOException, AuthorizationException {  
		...
		config.put(Config.NIMBUS_HOST, "storm-nimbus");
		config.put(Config.NIMBUS_THRIFT_PORT, 6627);
		config.put(Config.STORM_ZOOKEEPER_PORT, 2181);
		config.put(Config.STORM_ZOOKEEPER_SERVERS, Arrays.asList("zookeeper"));
	...
	private static StormTopology makeTopology() {
		...
		hTableConfig.setZkQuorum("zookeeper");
		...
		// Redis Bolt
		JedisPoolConfig jedisPoolConfig = new JedisPoolConfig.Builder().setHost("redis").setPort(6379).build();
